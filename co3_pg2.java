...
class employees{
	...
	String name,address;
	double salary;
	employees(int id,String name,String add,double sal){
		empid=id;
		this.name=name;
		this.address=add;
		this.salary=sal;
...
class teachers extends employees{
	...
	teachers(int id,String name,String add,double sal,String dep,int n,String sub[]) {
...
public class co3_pg2 {
	public static void main(String args[]) {
		...
		while(j<n) {
			...
			obj[j]=new teachers(empid,name,address,salary,dep,num,sub);
			j++;
		}
		j=0;
		while(j<n) {
